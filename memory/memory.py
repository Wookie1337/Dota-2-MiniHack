...
class Memory:
    def __init__(self, exe_name: str) -> None:
        ...
        if not self._initialize(exe_name):
            raise RuntimeError(f"Failed to initialize memory for process: {exe_name}")
            ...
            for proc in psutil.process_iter(["name"]):
                if proc.info["name"] == exe_name:
                    try:
                        ...
                        logger.info(f"[+] Connected to {exe_name} (PID: {self.process.process_id})")
                        return
                    except Exception as e:
                        logger.exception(f"[!] Error attaching to process: {e}")
            time.sleep(1)

    def _load_modules(self) -> None:
        required_modules = ["client.dll", "engine2.dll", "panorama.dll", "particles.dll"]
    ...
    def get_absolute_address(self, instr_ptr: int, offset: int = 3, size: int = 7) -> int:
        """
    ...
    def _normalize_bytes(self, pattern: str) -> bytes:
        return bytes(
            ''.join('\\x' + b if b not in {'??', '?'} else '.' for b in pattern.split()),
            encoding='latin-1'
        )

    def aob_scan_module(self, pattern: Union[str, list[str]], module: str) -> Optional[int]:
        """
        ...
        for pat in patterns:
            try:
                ...
                if result:
                    logger.debug(f"[✓] Pattern found in {module} at 0x{result:X}")
                    return result
            except Exception as e:
                logger.exception(f"[!] Pattern scan error: {e}")
    ...
    def patch(self, address: int, patch_bytes: bytes) -> None:
        ...
        if not address:
            ...
            return
        try:
            self.process.write_bytes(address, patch_bytes, len(patch_bytes))
